...
*/
	...
	//fp에서(descriptor) 0 in 1 out 2 error  3번으로 설정되는 이유
	//open예외처리 할것. -1이 나올 경우에 , O_NONBLOCK 으로 바꿈으로.
	fp=open(argv[1],O_RDONLY);
	rfp= open(argv[2],O_RDWR|O_CREAT|O_APPEND|O_SYNC|O_NONBLOCK|\
                          S_IRWXU|S_IWGRP|S_IRUSR|S_IRGRP|S_IROTH,0644);
	#if LOG ==1
	//??...생성된 파일이 이미 있는경우 -1반환됨...이유를 모르겠습니다.. 오픈 설정문제 예상중...
	//O_TRUNC 로 덮어쓰기로 해결 가능함. 이어쓰기는..? 어떤식으로??
